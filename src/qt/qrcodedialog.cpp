...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("TBN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Thebotcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TBN unit input in lnReqAmount, we generate the URI with TBN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::TBN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
